...
import { 
  ...
  export interface IAnalysisService {
    ...
    runAnalysis(analysisName: string, type: string): Promise<{ success: boolean; status: AnalysisStatus; logs: AnalysisLog[] }>;
    ...
    updateAnalysis(analysisName: string, content: string): Promise<AnalysisUpdateResult>;
    renameAnalysis(analysisName: string, newFileName: string): Promise<AnalysisUpdateResult>;
    
    // Logs
    addLog(analysisName: string, message: string): Promise<void>;
    getLogs(analysisName: string, page?: number, limit?: number): Promise<AnalysisLog[]>;
    getLogsForDownload(analysisName: string, timeRange: LogTimeRange): Promise<LogDownloadResult>;
    clearLogs(analysisName: string): Promise<{ success: boolean; message: string }>;
    
    // Environment
    getEnvironment(analysisName: string): Promise<EnvironmentVariables>;
    updateEnvironment(analysisName: string, env: EnvironmentVariables): Promise<AnalysisUpdateResult>;
    ...
    updateConnectionState(analysisName: string, state: ConnectionState): Promise<void>;
    initializeConnectionMonitor(analysisName: string, type: string): Promise<IConnectionMonitor>;
